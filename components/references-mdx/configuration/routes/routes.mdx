...
&#8203;<ProductName /> uses Routes to define the behavior of how a request is handled by the routing layer. For example, you might use a Route to proxy a URL to another, redirect a client, or apply a header with the response to a request.

By default, routing is defined by the filesystem of your deployment. For example, if a user makes a request to `/123.png`, and your [`vercel.json` file](/docs/configuration#project/routes) does not contain any routes with a valid `src` matching that path, it will fallback to the filesystem and serve `/123.png` if it exists.
...
A Route can be defined within a project's [`vercel.json` configuration file](/docs/configuration#project/routes) as an object within an array assigned to the `routes` property, like the following which creates a simple proxy from one path to another:

<Code lang="json">{`{
  "routes": [{ "src": "/about", "dest": "/about.html" }]
}`}</Code>
<Caption>
    An example <InlineCode>vercel.json</InlineCode> file with a{' '}
    <InlineCode>routes</InlineCode> property that proxies one path to another
    upon request.
  </Caption>

&#8203;<ProductName /> Routes have multiple properties for each route object that help define the behavior of a response to each request to a particular path.
...
The following example shows a `vercel.json` configuration that takes a `src` path and proxies it to a [destination `dest` path](#dest).
...
`dest` is used to proxy the [`src`](#src) path to another path, such as another URL or <ProductName /> hosted Serverless Function.
...
<Caption>
  An example <InlineCode>vercel.json</InlineCode> file with{' '}
  <InlineCode>routes</InlineCode> properties that proxy paths to another upon
  request.
</Caption>

<Note>
  You can point the <InlineCode>dest</InlineCode> path to any URL,{' '}
  <ProductName /> hosted Serverless Function, or even non <ProductName /> hosted
  URLs as shown in the code above. If you don't perform any proxying, you can
  safely remove <InlineCode>dest</InlineCode>.
</Note>
...
The `status` property defines the status code that <ProductName /> should respond with when a path is requested.
...
<Note>
  ...
  property can also point to non <ProductName /> hosted URLs.
</Note>
...
<Note>
  ...
  as an example of a <InlineCode>vercel.json</InlineCode> configuration that
  tells <ProductName /> to build JavaScript files with Node.js and outputs them
  as{' '}
...
<Caption>
  An incorrect example <InlineCode>vercel.json</InlineCode> file that will match
  all
...
<Caption>
  A correct example <InlineCode>vercel.json</InlineCode> file that will match
  all
...
If you are using a Next.js app and want to learn more about using custom routes with <ProductName />, read our guide:

<Card
  title="Deploying with Custom Next.js Routes"
  href="/guides/custom-next-js-server-to-routes"
>
  Custom Routes with Next.js and <ProductName />.
</Card>
...
<Caption>
  A <InlineCode>vercel.json</InlineCode> file where filesystem routes are
  explicitly defined.
</Caption>
...
<Caption>
  A <InlineCode>vercel.json</InlineCode> file, using handle filesystem to route
  to filesystem routes.
</Caption>
...
If you are not using a framework, you can emit a `404.html` file to the [Output Directory](/docs/v2/build-step#output-directory).
...
<Caption>
  A <InlineCode>vercel.json</InlineCode> file, assigning
  <InlineCode>other-404.html</InlineCode> as the 404 page for any path not matched
  in the filesystem.
</Caption>

In the snippet above, precedence is given to the filesystem, routing only to `/other-404.html` when no other file matches the requested path.
...
<Caption>
  A <InlineCode>vercel.json</InlineCode> file, setting a{' '}
  <InlineCode>404</InlineCode> status code for the{' '}
  <InlineCode>/stats</InlineCode> path.
</Caption>

In the snippet above, visiting the `/stats` path will respond with a `404` status code.
...
<Caption>
  A <InlineCode>vercel.json</InlineCode> file, providing a fallback to{' '}
...
<Caption>
  A <InlineCode>vercel.json</InlineCode> file, providing a{' '}
...
<Caption>
  A <InlineCode>vercel.json</InlineCode> file, expanded to show the effect of
  using the filesystem to handle routing.
</Caption>
...
<Caption>
  A <InlineCode>vercel.json</InlineCode> file that will continue matching routes
...
<Caption>
  A <InlineCode>vercel.json</InlineCode> file using{' '}
...
<Caption>
  An incorrect example of a <InlineCode>vercel.json</InlineCode> route.
</Caption>
...
<Caption>
  A correct example of a <InlineCode>vercel.json</InlineCode> route.
</Caption>
...
<Caption>
  A <InlineCode>vercel.json</InlineCode> route using a catch-all except regex.
</Caption>
...
<Caption>
  A <InlineCode>vercel.json</InlineCode> route using a negative lookahead.
</Caption>
